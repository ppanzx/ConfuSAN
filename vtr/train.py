...
from tvr.utils.metrics import compute_metrics, otnorm, isnorm, tensor_text_to_video_metrics, tensor_video_to_text_sim
...
def get_args(description='Disentangled Representation Learning for Text-Video Retrieval'):
    ...
    parser.add_argument('--agg_module', type=str, default="seqTransf", choices=["None", "mlp", "seqLSTM", "seqTransf"],
    ...
    parser.add_argument('--tau', type=float, default=-1, help='tau of InfoNCE loss.')
    parser.add_argument('--hubnorm', type=str, default=None,choices=[None, "ot", "is", "otis"],help='Do not normalize the similarity matrix.')
...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer, scheduler, global_step, max_steps):
    ...
    if epoch>10:model.module.loss_fct.max_violation_on()
    else:model.module.loss_fct.max_violation_off()
    model.train()
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        if hasattr(model, 'module'):
            torch.clamp_(model.module.clip.logit_scale.data, max=np.log(1000))
            logit_scale = model.module.clip.logit_scale.exp().item()
        else:
            torch.clamp_(model.clip.logit_scale.data, max=np.log(1000))
...
def eval_epoch(args, model, test_dataloader, device):
    ...
    with torch.no_grad():
        ...
        if multi_sentence_:  # multi-sentences retrieval means: one clip has two or more descriptions.
            batch_feat_v = batch_feat_v[cut_off_points_]
            batch_mask_v = batch_mask_v[cut_off_points_]
    ...
    if multi_sentence_:
        ...
        if not hasattr(args,"hubnorm") or args.hubnorm is None:
            tv_metrics = compute_metrics(sim_matrix)
            vt_metrics = compute_metrics(sim_matrix.T)
        elif args.hubnorm=="ot":
            sim_matrix = otnorm(sim_matrix)       
            tv_metrics = compute_metrics(sim_matrix)
            vt_metrics = compute_metrics(sim_matrix.T)
        elif args.hubnorm=="is":
            sim_matrix_i2t = sim_matrix * isnorm(sim_matrix, tau=0.01, axis=0)
            sim_matrix_t2i = sim_matrix * isnorm(sim_matrix, tau=0.01, axis=1)
            tv_metrics = compute_metrics(sim_matrix_i2t)
            vt_metrics = compute_metrics(sim_matrix_t2i.T)
        else:
            ## baseline 
            tv_metrics = compute_metrics(sim_matrix)
            vt_metrics = compute_metrics(sim_matrix.T)
