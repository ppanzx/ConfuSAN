...
from .module_clip import CLIP, convert_weights, _PT_NAME, _download, _MODELS
from .module_cross import CrossModel, Transformer as TransformerClip, Crossformer
from .module_cross import NMFormer, PyramidNMF, sparsemax, Wasserstain
from .until_module import LayerNorm, AllGather, AllGather2, CrossEn, MixLoss
...
class DRL(nn.Module):
    def __init__(self, config):
        ...
        model_path = _download(_MODELS[backbone])
        ...
        self.logit_scale = self.clip.logit_scale.exp() if config.tau==-1 else config.tau
        self.apply(self.init_weights)  # random init must before loading pretrain
        self.clip.load_state_dict(state_dict, strict=False)

        self.frame_weight_fc = nn.Linear(config.max_frames, config.max_frames, bias=False)
                ...
                for key, val in state_dict.items():
                    ...
                    if key.find("transformer.resblocks") == 0:
                        ...
                        if num_layer < cross_config.num_hidden_layers:
                            ...
                            continue
        elif self.interaction == 'ctf':
            self.crossformer = Crossformer(cross_config.hidden_size, 1, 0.0)
        elif self.interaction == 'ot':
            self.ot = Wasserstain()
        elif self.interaction == 'nmformer':
            self.nmformer = NMFormer(cross_config.hidden_size, 0.4)
        elif self.interaction == 'pyramid':
            self.pyramid = PyramidNMF(cross_config.hidden_size)
        if self.agg_module in ["seqLSTM", "seqTransf"]:
        ...
        self.ln1d = nn.LayerNorm(embed_dim)
    ...
    def forward(self, text_ids, text_mask, video, video_mask=None):
        ...
        # text_feat = text_feat.to(self.loss_device)
        # video_feat = video_feat.to(self.loss_device)
        if self.training:
            sim_matrix, cdcr_loss = self.get_similarity_logits(text_feat, video_feat,
    ...
    def dp_interaction(self, text_feat, video_feat, text_mask, video_mask):
        ...
        if self.training:
            ...
            if self.config.cdcr != 0:
                ...
                return retrieve_logits, cdcr_loss
            else:
                return retrieve_logits, 0.0
        else:
            return retrieve_logits, 0.0
    ...
    def nmf_interaction(self, text_feat, video_feat, text_mask, video_mask):
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        sims = torch.einsum("vfd,td->vtf", video_feat, text_feat) # Bi x Bt x K x L
        ...
        retrieve_logits = torch.einsum("vtf,vtf->tv", sims, attn)
        # retrieve_logits = torch.max(least_angle, selection)
        # retrieve_logits = torch.einsum("vtf,vtf->tv", sims, attn)
        if self.training:
            # logit_scale = self.clip.logit_scale.exp() #
            # retrieve_logits = logit_scale * retrieve_logits

            D = video_feat.shape[-1]
            v_feat = self.ln1d(video_feat)
            c = torch.einsum("vfm,vfn->vmn",[v_feat,v_feat])/D 
            mask = torch.eye(D).unsqueeze(dim=0).repeat(c.shape[0],1,1) > .5
            c[mask.to(c.device)] = 0
            off_diag = c.pow_(2).sum()
            cdcr_loss = off_diag * self.config.cdcr_alpha1
            return retrieve_logits, cdcr_loss
        else:
            return retrieve_logits, 0.0
        
    def nmfv2_interaction(self, text_feat, video_feat, text_mask, video_mask):
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        # attn_tilde = torch.rand_like(sims).to(sims.device)
        # if False:
        #     attn_tilde = F.gumbel_softmax(sims, dim=0, hard=True)
        # else:
        #     ind = sims.max(dim=-1,keepdim=True)[1]
        #     attn_tilde = torch.arange(sims.shape[-1]).expand_as(sims).to(sims.device)
        #     attn_tilde = (attn_tilde==ind.long()).float()

        self.iters = 5
        self.lamb = 15e-2
        self.eps = 1e-6
        margin=-0
        # sims = sims.masked_fill(sims <= margin, self.eps)
        eta = self.lamb/self.iters
        for i in range(self.iters):
            dnm = torch.einsum("vmd,vnd,vtn->vtm", video_feat, video_feat, attn_tilde) + self.eps # denominator
            attn_tilde = attn_tilde - eta*(dnm-sims)
            attn = attn_tilde.masked_fill(attn_tilde <= margin, self.eps)
            norm = torch.einsum("vtf,vfd->vtd", attn_tilde, video_feat).norm(p=2, dim=-1, keepdim=True)
            attn =  attn_tilde/norm # 
            if (sims-dnm).abs().max()<self.eps:
                break
    ...
    def cca_interaction(self, text_feat, video_feat, text_mask, video_mask):
        if self.training and torch.cuda.is_available():  # batch merge here
            text_feat = allgather(text_feat, self.config)
            video_feat = allgather(video_feat, self.config)
            text_mask = allgather(text_mask, self.config)
        ...
        # initialize the attention matrix
        mask = torch.einsum("vf,tl->vtfl", video_mask, text_mask)
        sims = torch.einsum("vfd,tld->vtfl", video_feat, text_feat)
        phi_tilde = mask.sum(dim=[-1])!=0  # Bi x Bt x K
        psi_tilde = mask.sum(dim=[-2])!=0  # Bi x Bt x L
        phi_tilde = phi_tilde*(1/phi_tilde.sum(dim=-1, keepdim=True))
        psi_tilde = psi_tilde*(1/psi_tilde.sum(dim=-1, keepdim=True))
        phi = phi_tilde.clone()
        psi = psi_tilde.clone()
        ...
        eta = self.lamb/self.iters
        for i in range(self.iters):
            phi_tilde = phi_tilde + eta*torch.einsum("vtfl,vtl->vtf", sims, psi)
            psi_tilde = psi_tilde + eta*torch.einsum("vtfl,vtf->vtl", sims, phi)

            norm = torch.einsum("vtf,vfd->vtd", phi_tilde, video_feat).norm(p=2, dim=-1, keepdim=True)
            phi =  phi_tilde/norm # 
            phi[phi<=margin] = self.eps

            norm = torch.einsum("vtl,tld->vtd", psi_tilde, text_feat).norm(p=2, dim=-1, keepdim=True)
            psi =  psi_tilde/norm # 
            psi[psi<=margin] = self.eps
            
        retrieve_logits = torch.einsum("vtfl,vtf,vtl->tv", sims, phi, psi, )
        if self.training:
            # logit_scale = self.clip.logit_scale.exp() #
            # retrieve_logits = logit_scale * retrieve_logits
            return retrieve_logits, 0.0
    ...
    def ca_interaction(self, text_feat, video_feat, text_mask, video_mask):
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        sims = torch.einsum("td,vfd->tvf", text_feat, video_feat)
        attn = sims.clone()
        attn = attn.masked_fill(sims<0, -1)
        attn = F.softmax(attn/0.1, dim=-1)
        # attn = attn.masked_fill(sims<0, 0)

        video_embeds_pooled = torch.einsum("tvf,vfd->vtd", attn, video_feat)
        video_embeds_pooled = video_embeds_pooled / video_embeds_pooled.norm(dim=-1, keepdim=True)
        retrieve_logits = torch.einsum("vtd,td->tv", video_embeds_pooled, text_feat)
    ...
    def ctf_interaction(self, text_feat, video_feat, text_mask, video_mask):
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        video_embeds_pooled = self.crossformer(text_feat,video_feat)
        video_embeds_pooled = video_embeds_pooled / video_embeds_pooled.norm(dim=-1, keepdim=True)
        retrieve_logits = torch.einsum("vtd,td->tv", video_embeds_pooled, text_feat)
    ...
    def nmformer_interaction(self, text_feat, video_feat, text_mask, video_mask):
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        video_embeds_pooled = self.nmformer(text_feat,video_feat)
        video_embeds_pooled = video_embeds_pooled / video_embeds_pooled.norm(dim=-1, keepdim=True)
        retrieve_logits = torch.einsum("vtd,td->tv", video_embeds_pooled, text_feat)
    ...
    def ot_interaction(self, text_feat, video_feat, text_mask, video_mask):
        if self.training and torch.cuda.is_available():  # batch merge here
            text_feat = allgather(text_feat, self.config)
            video_feat = allgather(video_feat, self.config)
            text_mask = allgather(text_mask, self.config)
        ...
        retrieve_logits = self.ot(text_feat, video_feat, text_mask, video_mask)
    ...
    def pm_interaction(self, text_feat, video_feat, text_mask, video_mask):
        ## positive mean
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        sims = torch.einsum("vfd,td->vtf", video_feat, text_feat) # Bi x Bt x K x L
        video_feat_pm = video_feat.unsqueeze(dim=1).repeat(1,text_feat.shape[0],1,1)

        video_feat_pm = video_feat_pm.masked_fill(sims.unsqueeze(dim=-1)<0, 1e-6)
        video_feat_pm = video_feat_pm.sum(dim=-2)/((sims>0).sum(dim=-1,keepdim=True)+1e-6)

        video_feat_pm = video_feat_pm / video_feat_pm.norm(dim=-1, keepdim=True)
        retrieve_logits = torch.einsum("vtd,td->tv", video_feat_pm, text_feat)
    ...
    def nmfti_interaction(self, text_feat, video_feat, text_mask, video_mask):
        if self.training and torch.cuda.is_available():  # batch merge here
            text_feat = allgather(text_feat, self.config)
            video_feat = allgather(video_feat, self.config)
            text_mask = allgather(text_mask, self.config)
        ...
        sims = torch.einsum('atd,bvd->abtv', [text_feat, video_feat])
        sims = torch.einsum('abtv,at->abtv', [sims, text_mask])
        sims = torch.einsum('abtv,bv->abtv', [sims, video_mask])
        text_weight = (1/text_mask.sum(-1,keepdim=True))*text_mask
        video_weight = (1/video_mask.sum(-1,keepdim=True))*video_mask
        ...
        eta = self.lamb/self.iters
        for i in range(self.iters):
            dnm = torch.einsum("vmd,vnd,tvln->tvlm", [video_feat, video_feat, attn_tilde]) + self.eps # denominator
            attn_tilde = attn_tilde - eta*(dnm-sims)
            attn_tilde[attn_tilde <= margin] = self.eps
            norm = torch.einsum("tvlf,vfd->tvld", [attn_tilde, video_feat]).norm(p=2, dim=-1, keepdim=True)
            attn =  attn_tilde/norm # 
            if (sims-dnm).abs().max()<self.eps:
                break
        t2v_logits = torch.einsum("tvlf,tvlf->tvl", sims, attn)
        t2v_logits = torch.einsum('tvl,tl->tv', [t2v_logits, text_weight])
        
        attn_tilde = sims.clone()
        for i in range(self.iters):
            dnm = torch.einsum("tmd,tnd,tvnf->tvmf", [text_feat, text_feat, attn_tilde]) + self.eps # denominator
            attn_tilde = attn_tilde - eta*(dnm-sims)
            attn_tilde[attn_tilde <= margin] = self.eps
            norm = torch.einsum("tvlf,tld->tvfd", [attn_tilde, text_feat]).norm(p=2, dim=-1, keepdim=True)
            attn = (attn_tilde.permute(0,1,3,2)/norm).permute(0,1,3,2) # 
            if (sims-dnm).abs().max()<self.eps:
                break
        v2t_logits = torch.einsum("tvlf,tvlf->tvf", sims, attn)
        v2t_logits = torch.einsum('tvf,vf->tv', [v2t_logits, video_weight])
        retrieve_logits = (t2v_logits + v2t_logits) / 2.0
        if self.training:
            # logit_scale = self.clip.logit_scale.exp() #
            # retrieve_logits = logit_scale * retrieve_logits
    ...
    def pyramid_interaction(self, text_feat, video_feat, text_mask, video_mask):
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        retrieve_logits = self.pyramid(text_feat, video_feat)   
    ...
    def avepool_interaction(self, text_feat, video_feat, text_mask, video_mask):
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        retrieve_logits = torch.einsum("td,vd->tv", text_feat, video_feat)
    ...
    def maxpool_interaction(self, text_feat, video_feat, text_mask, video_mask):
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        video_feat = video_feat.max(dim=1)[0]
        video_feat = video_feat / video_feat.norm(dim=-1, keepdim=True)           
    ...
    def sparsemax_interaction(self, text_feat, video_feat, text_mask, video_mask):
        ## positive mean
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        sims = torch.einsum("vfd,td->tvf", video_feat, text_feat) # Bi x Bt x K x L

        attn = sparsemax(sims)
        retrieve_logits = torch.einsum("tvf,tvf->tv", sims, attn)
    ...
    def maxframe_interaction(self, text_feat, video_feat, text_mask, video_mask):
        ## positive mean
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        sims = torch.einsum("vfd,td->tvf", video_feat, text_feat) # Bi x Bt x K x L
        # sims = F.softmax(sims/0.07, dim=-1)
        retrieve_logits = sims.max(dim=-1)[0]
        # retrieve_logits = torch.logsumexp(sims,-1)
    ...
    def simsmax_interaction(self, text_feat, video_feat, text_mask, video_mask):
        if self.training and torch.cuda.is_available():  # batch merge here
            text_feat = allgather(text_feat, self.config)
            video_feat = allgather(video_feat, self.config)
            text_mask = allgather(text_mask, self.config)
        ...
        text_glo = text_feat[:,0]
        text_glo = text_glo / text_glo.norm(dim=-1, keepdim=True)  # B x D

        video_feat = video_feat / video_feat.norm(dim=-1, keepdim=True)

        sims = torch.einsum("vfd,td->tvf", video_feat, text_glo) # Bi x Bt x K x L
        # sims = F.softmax(sims/0.07, dim=-1)
        retrieve_logits = sims.max(dim=-1)[0]
        # retrieve_logits = torch.logsumexp(sims,-1)
    ...
    def isa_interaction(self, text_feat, video_feat, text_mask, video_mask):
        ## positive mean
        text_feat = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        ...
        sims = torch.einsum("vfd,td->tvf", video_feat, text_feat) # Bi x Bt x K 

        attn = torch.softmax(self.frame_weight_fc(torch.softmax(sims/1e-2, dim=-1))/1e-2, dim=-1)        
        retrieve_logits = torch.einsum("tvf,tvf->tv", sims, attn)
    ...
    def joint_interaction(self, text_feat, video_feat, text_mask, video_mask):
        if self.training and torch.cuda.is_available():  # batch merge here
            text_feat = allgather(text_feat, self.config)
            video_feat = allgather(video_feat, self.config)
            text_mask = allgather(text_mask, self.config)
        ...
        text_glo = self.get_text_sep_feat(text_feat, text_mask)  # B x 1 x D
        text_glo = text_glo.squeeze()  # B x 1 x D -> B x D
        text_glo = text_glo / text_glo.norm(dim=-1, keepdim=True)  # B x D
        video_feat = video_feat / video_feat.norm(dim=-1, keepdim=True)
        text_feat = text_feat / text_feat.norm(dim=-1, keepdim=True)
        ...
        loc = (t2v_logits + v2t_logits) / 2.0

        ## maxframe selection
        sims = torch.einsum("vfd,td->tvf", video_feat, text_glo) # Bi x Bt x K x L
        glo = sims.max(dim=-1)[0]

        # retrieve_logits = loc

        if self.training:
            return t2v_logits, 0.0
        else:
            return t2v_logits, 0.0
    ...
    def get_similarity_logits(self, text_feat, video_feat, text_mask, video_mask, shaped=False):
        ...
        if self.interaction == 'dp':
            t2v_logits, cdcr_loss = self.dp_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'xti':
            t2v_logits, cdcr_loss = self.xti_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction in ['ti', 'wti']:
            t2v_logits, cdcr_loss = self.wti_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'nmf':
            t2v_logits, cdcr_loss = self.nmf_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'nmfv2':
            t2v_logits, cdcr_loss = self.nmfv2_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'cca':
            t2v_logits, cdcr_loss = self.cca_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'ca':
            t2v_logits, cdcr_loss = self.ca_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'ctf':
            t2v_logits, cdcr_loss = self.ctf_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'ot':
            t2v_logits, cdcr_loss = self.ot_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'pm':
            t2v_logits, cdcr_loss = self.pm_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'nmfti':
            t2v_logits, cdcr_loss = self.nmfti_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'nmformer':
            t2v_logits, cdcr_loss = self.nmformer_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'pyramid':
            t2v_logits, cdcr_loss = self.pyramid_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'maxpool':
            t2v_logits, cdcr_loss = self.maxpool_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'avepool':
            t2v_logits, cdcr_loss = self.avepool_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'maxframe':
            t2v_logits, cdcr_loss = self.maxframe_interaction(text_feat, video_feat, text_mask, video_mask)        
        elif self.interaction == 'sparsemax':
            t2v_logits, cdcr_loss = self.sparsemax_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'isa':
            t2v_logits, cdcr_loss = self.isa_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'joint':
            t2v_logits, cdcr_loss = self.joint_interaction(text_feat, video_feat, text_mask, video_mask)
        elif self.interaction == 'simsmax':
            t2v_logits, cdcr_loss = self.simsmax_interaction(text_feat, video_feat, text_mask, video_mask)
        else:
            raise NotImplementedError
        return t2v_logits, cdcr_loss
