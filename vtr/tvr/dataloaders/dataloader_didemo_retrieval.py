...
class DiDeMoDataset(Dataset):
    ...
    def _get_text(self, video_id, sub_id):
        ...
        starts = np.zeros(k, dtype="long")
        ends = np.zeros(k, dtype="long")
        pairs_text = np.zeros((k, self.max_words), dtype="long")
        pairs_mask = np.zeros((k, self.max_words), dtype="long")
        pairs_segment = np.zeros((k, self.max_words), dtype="long")
    ...
    def _get_rawvideo(self, idx, s, e):
        video_mask = np.zeros((len(s), self.max_frames), dtype="long")
