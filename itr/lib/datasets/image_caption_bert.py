...
class RawImageDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, tokenzier, preprocess, opt, train):
        self.opt = opt
        self.train = train
...
def get_loader(data_path, data_name, data_split, tokenizer, preprocess, opt, batch_size=100,
    ...
    if opt.precomp_enc_type in ["backbone","clip"]:
        dset = RawImageDataset(data_path, data_name, data_split, tokenizer, preprocess, opt, train)
...
def get_train_loader(data_path, data_name, tokenizer, preprocess, batch_size, workers, opt, shuffle):
