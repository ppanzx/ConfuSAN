...
class RawImageDataset(data.Dataset):
    ...
    def __init__(self, data_path, data_name, data_split, vocab, opt, train):
        ...
        if self.num_images != self.num_captions:
            self.im_div = 5
        else:
            self.im_div = 1
    ...
    def __getitem__(self, index):
        cap_ids = list(range(index*self.im_div,(index+1)*self.im_div))
        captions = [self.captions[id] for id in cap_ids]

        # Convert caption (string) to word ids (with Size Augmentation at training time).
        targets = [process_caption(self.vocab, caption, self.train) for caption in captions]
        ...
        return image, targets, img_id, cap_ids
...
def collate_fn(data):
    ...
    images, captions, img_ids, cap_ids = zip(*data)
    img_lengths = [len(image) for image in images]

    captions = [item for sublist in captions for item in sublist]
    cap_ids = [item for sublist in cap_ids for item in sublist]
    captions, cap_ids = zip(*sorted(list(zip(captions, cap_ids)), key=lambda x: len(x[0]), reverse=True))
    # Merge images
    ...
    return all_images, img_lengths, targets, lengths, img_ids, cap_ids
