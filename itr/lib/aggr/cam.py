...
# cross-attention V3
# proposed by <cross modal sinkhorn matching>
class VSACoding(nn.Module):
    def __init__(self, temperature = 0.1, split=16):
        super(VSACoding,self).__init__()
        self.split = split
        self.temperature = temperature

    def VSAC(self, imgs, caps, img_lens, cap_lens):
        max_r,max_w = int(img_lens.max()),int(cap_lens.max())
        imgs = imgs[:,:max_r,:]
        caps = caps[:,:max_w,:]
        ...
        sims = nn.LeakyReLU(0.1)(sims)
        sims = sims.masked_fill(mask==0, 0)
        sims = F.normalize(sims, p=2, dim=-1) # l2 normlization for sims

        sims = sims.masked_fill(mask==0, INF)
        sims = torch.softmax(sims/self.temperature, dim=-2)
        sims = sims.masked_fill(mask == 0, 0)

        sims = torch.einsum("itkl,ikd->itld", sims, imgs)
        sims = F.normalize(sims, p=2, dim=-1)
        sims = torch.einsum("itld,tld->itl", sims, caps)

        sim_mask = cap_mask.permute(2, 0, 1).repeat(imgs.size(0),1,1)
        sims = sims.masked_fill(sim_mask==0, MASK) # Bi x Bt x L
        return sims

    def forward(self, imgs, caps, img_lens, cap_lens):
        if not self.training:
            return self.VSAC(imgs, caps, img_lens, cap_lens)
        ...
        for i in range(self.split):
            ...
            sims[:,beg:ed,:max_w] = self.VSAC(imgs, caps[beg:ed], img_lens, cap_lens[beg:ed])
        return sims


class CollaborativeCoding(nn.Module):
    def __init__(self, tau = 0.1, split=16):
        super(CollaborativeCoding,self).__init__()
        self.split = split
        self.tau = tau
        self.eps = 1e-6

    def collaborative(self, imgs, caps, img_lens, cap_lens):
        max_r,max_w = int(img_lens.max()),int(cap_lens.max())
        imgs = imgs[:,:max_r,:]
        caps = caps[:,:max_w,:]
        ...
        ## W' = (XX'+λI)^{-1}XY'
        sims = torch.einsum("imd,ind->imn", imgs, imgs) + \
            self.tau*torch.eye(K).unsqueeze(dim=0).repeat(Bi,1,1).to(imgs.device) # 
        # sims = torch.einsum("imd,ind->imn", imgs, imgs).float() # pseudo-inverse

        sims = torch.linalg.inv(sims)
        sims = torch.einsum("ikk,ikd,tld->itkl", sims, imgs, caps) # Bi x Bt x K x L 
        ...
        ## 
        sims = torch.einsum("itkl,ikd->itld", attn, imgs)
        sims = F.normalize(sims, p=2, dim=-1)
        sims = torch.einsum("itld,tld->itl", sims, caps)
        sims = sims/(attn.norm(p=2,dim=-2)+self.eps)
    ...
    def forward(self, imgs, caps, img_lens, cap_lens):
        if not self.training:
            return self.collaborative(imgs, caps, img_lens, cap_lens)
        n_image,_,_ = imgs.shape
        n_caption,max_len,_ = caps.shape
        sims = torch.zeros(n_image,n_caption,max_len).to(device=caps.device)+MASK
        step = n_caption//self.split
        for i in range(self.split):
            ...
            if beg>=ed:break
            max_w = int(cap_lens[beg:ed].max())
            sims[:,beg:ed,:max_w] = self.collaborative(imgs, caps[beg:ed], \
                                img_lens, cap_lens[beg:ed])
        return sims
...
class Projection(nn.Module):
    ...
    def forward(self, imgs, caps, img_lens, cap_lens):
        ...
        for i in range(self.iters):
            ...
            dnm = torch.einsum("imn,itnd->itmd", dnm, attn)
            attn = attn*sims/dnm
            attn[attn<0]=0
