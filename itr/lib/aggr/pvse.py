...
if __name__=="__main__":
    ...
    model = MultiHeadSelfAttention(n_head=8, d_in=1024, d_hidden=512)
    import sys
    sys.path.insert(0,"/home/panzx/retrieval/CLIP4OMIT/")
    from lib.modules.utils import get_mask
    img_mask = get_mask(img_lens).squeeze()
    max_r,max_w = int(img_lens.max()),int(cap_lens.max())
    imgs = imgs[:,:max_r,:]
    output, attn = model(imgs, img_mask)
    pass
