...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, text_embeds, video_embeds, mask=None):
        """
        Input
        ...
        if mask is not None:
            mask = mask.unsqueeze(dim=-1).repeat(1,1,1,self.num_heads).permute(0,3,2,1)
            attention_logits = attention_logits.masked_fill(mask==0, float("-inf"))
        attention_logits = attention_logits / math.sqrt(self.head_dim)
        attention_weights = F.softmax(attention_logits, dim=2)
        # attention_logits = attention_logits.masked_fill(mask==0, 0)
...
class CrossTransformer(nn.Module):
    ...
    def forward(self, text_embeds, video_embeds, mask=None):
        ...
        # num_vids x num_texts x embed_dim
        attn_out = self.cross_attn(text_embeds, video_embeds, mask)
...
class CTF(nn.Module):
    def __init__(self, embed_dim, num_heads=8, dropout=0.0):
        ...
        self.crosstranf = CrossTransformer(embed_dim, num_heads, dropout)  

    def gatter(self, embeds, mask):
        return

    def forward(self, imgs, caps, img_lens, cap_lens,):
        query = caps[torch.arange(caps.shape[0]), cap_lens.long()-1]
        attn_mask = get_mask(img_lens).repeat(1,1,caps.shape[0]).permute(0,2,1)
        query_p = self.crosstranf(query, imgs, attn_mask)
        query_p = query_p / query_p.norm(dim=-1, keepdim=True)
        sims = torch.einsum("vtd,td->vt", query_p, query)
        return sims