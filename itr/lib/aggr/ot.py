...
class Wasserstain(nn.Module):
    ...
    def Sinkhorn_Knopp(self, sims, r, c):
        ...
        for i in range(self.iters):
            v0 = v
            u = c/torch.einsum("itkl,itk->itl",P,v) # u(0)
            v = r/torch.einsum("itkl,itl->itk",P,u)
            err = (v - v0).abs().mean()
            if err.item() < self.eps:
                break
        P = torch.einsum("itk,itl->itkl",v,u)*P
        return P

    def forward(self, img_cls, imgs, cap_cls, caps, img_lens, cap_lens,return_attn=False):
        ...
        for i in range(self.split):
            ...
            if self._init==0:
                ...
                raise ValueError
            tp = self.Bregman((1-fg_sims[beg:ed]).masked_fill(mask[beg:ed] == 0, INF), r, c)
