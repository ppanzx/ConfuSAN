...
if __name__ == '__main__':
    ctm = CTM(sample_ratio=0.25, embed_dim=1024, dim_out=1024, k=3)
    block = TCBlock(dim=1024, num_heads=8)
    x = torch.randn((128, 36, 1024))
    idx_token = torch.arange(36)[None, :].repeat(128, 1)
    agg_weight = x.new_ones(128, 36, 1)
    from utils import get_mask
    lens = torch.randint(low=1, high=36, size=[128])
    lens[0]=36
    ...
    print(q_dict["x"].size())
    ctm1 = CTM(sample_ratio=0.125, embed_dim=1024, dim_out=1024, k=3)
    block1 = TCBlock(dim=1024, num_heads=16)
