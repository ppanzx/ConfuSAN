...
from lib.datasets import image_caption_bigru_1v5,image_caption_bert, image_caption_clip
from lib.model import Model
from lib.evaluation import i2t, t2i, AverageMeter, LogCollector, otnorm, isnorm, encode_data, compute_sims
...
def main():
    ...
    if opt.text_enc_type=="bigru":
        ...
        if opt.wemb_type is not None:
            ...
            opt.word2idx = None
        # dataloader 
        train_loader, val_loader = image_caption_bigru_1v5.get_loaders(
    ...
    if opt.num_epochs==0:
        save_checkpoint({
            ...
            'opt': opt,
            'Eiters': 0,
        }, True, filename='checkpoint.pth'.format(0), prefix=opt.model_name + '/')
...
def train(opt, train_loader, model, epoch, val_loader):
    ...
    for i, train_data in enumerate(train_loader):
        ...
        # Update the model
        images, img_lengths, captions, lengths, img_ids, cap_ids = train_data
        if opt.precomp_enc_type in ["backbone","clip"] and epoch == opt.embedding_warmup_epochs:
            warmup_alpha = float(i) / num_loader_iter
            model.train_emb(images, captions, lengths, image_lengths=img_lengths, matching=matching, warmup_alpha=warmup_alpha)
        else:
            matching = torch.tensor(img_ids).unsqueeze(dim=-1)==(torch.tensor(cap_ids).unsqueeze(dim=0)//5)
            model.train_emb(images, captions, lengths, image_lengths=img_lengths, matching=matching)
...
def validate(opt, val_loader, model):
    ...
    imgs_cls, img_embs, caps_cls, cap_embs, img_lens, cap_lens = encode_data(model, val_loader, opt.log_step, logging.info)

    imgs_cls = np.array([imgs_cls[i] for i in range(0, len(imgs_cls), 5)])
    img_embs = np.array([img_embs[i] for i in range(0, len(img_embs), 5)])

    start = time.time()
    sims = compute_sims(imgs_cls, img_embs, caps_cls, cap_embs, img_lens, cap_lens, model)
    end = time.time()
    ...
    if not hasattr(opt,"hubnorm") or opt.hubnorm is None:
        ...
        (r1, r5, r10, medr, meanr) = i2t(npts, sims_i2t)
        sims_t2i = sims * isnorm(sims, tau=1, axis=1)
        (r1i, r5i, r10i, medri, meanr) = t2i(npts, sims_t2i)
    (r1, r5, r10, medr, meanr) = i2t(npts, sims)
    (r1i, r5i, r10i, medri, meanr) = t2i(npts, sims)
